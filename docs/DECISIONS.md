...
**Pendência registrada:** as duas chamadas a `getUser()` por requisição (uma no middleware, outra em `getCurrentProfile()`) são redundantes — o middleware já valida a sessão antes de deixar a requisição passar. Repassar o `user.id` já resolvido via header da requisição eliminaria a segunda chamada de rede inteira, cortando a latência pela metade, mas é uma mudança maior (toca middleware + todo layout/página protegido) que não foi feita durante o incidente em produção por segurança — fica como candidata a uma passada de otimização calma, fora de um incidente ativo.
