...
### DEC-053 — Renomeação dos programas Makarios

**Data:** 2026-07-15
**Decisão:** Nova migration atualiza só o `name` de exibição em `training_programs`: "Makarios 1" → "Makarios - Vol. 1 Essência", "Makarios 2" → "Makarios - Vol. 2 Caminho", "Makarios 3" → "Makarios - Vol. 3 Voz" (Cultura Emaús inalterado). Os códigos (`makarios_1`/`makarios_2`/`makarios_3`) não mudam — toda a lógica de elegibilidade (`lib/business-rules/eligibility.ts`) e validação (`lib/validations/training.ts`) já trabalha em cima do `code`, nunca do `name`, então não houve nenhuma mudança de código TypeScript, só dado.
**Motivo:** Pedido do responsável pelo produto.

### DEC-054 — "Serviço" renomeado para "Grupos de Atuação", catálogo substituído, multi-seleção
...
### DEC-057 — Produção quebrada (continuação): `supabase.auth.getUser()` trava indefinidamente no middleware com cookie de sessão obsoleto

**Data:** 2026-07-15
**Decisão:** `middleware.ts` agora envolve as duas chamadas de rede ao Supabase (`auth.getUser()` e a leitura de `profiles.role/active` para usuário logado em rota pública) num `withTimeout()` de 5s — se a chamada não responder nesse prazo, trata como "sem sessão"/"sem perfil" (mesmo comportamento já existente para erro de rede), em vez de deixar a Edge Function pendurada até o limite da plataforma.
**Motivo:** Depois de corrigir o `@opentelemetry/api` (DEC-056) e confirmar que as migrations e as credenciais do Supabase (URL e anon key) estavam corretas em produção, o log de runtime da Netlify mostrou o `/admin` retornando 500 após **11755ms**, com o `Next.js Middleware Handler` consumindo praticamente todo esse tempo sem log de erro — sintoma de uma chamada de rede pendurada, não de uma exceção lançada. Uma issue conhecida do `supabase-js` (github.com/supabase/supabase/issues/35754) descreve exatamente esse comportamento: `getUser()` pode travar indefinidamente quando o cookie de sessão está obsoleto/inválido, e o workaround recomendado pelos próprios mantenedores é envolver a chamada num timeout — não existe correção de versão que elimine a causa raiz até o momento desta decisão. Como o `try/catch` já existente em `middleware.ts` só protege contra exceção lançada, nunca contra uma Promise que nunca resolve, ele não ajudava nesse cenário.

### DEC-058 — Produção quebrada (continuação): `requireRole` deixa de delegar para `requireAuth`

**Data:** 2026-07-15
**Decisão:** `requireRole()` (`lib/auth/server.ts`) não chama mais `await requireAuth()` internamente — agora repete a checagem (perfil existe, ativo, papel permitido) e chama `redirect()` diretamente no próprio corpo, no mesmo nível de aninhamento que `requireAuth()` já usava sozinha.
**Motivo:** O erro genérico em `/admin` persistiu com o mesmo digest mesmo depois da correção do `@opentelemetry/api` (DEC-056), das migrations aplicadas em produção e do timeout no `getUser()` do middleware (DEC-057) — e o painel de observabilidade da Netlify mostrou que a falha, embora agora rápida (~1.2s, não mais um travamento de ~11s), continuava determinística e idêntica a cada tentativa. Isso aponta para uma exceção lançada de forma consistente, não uma falha de rede. Há um bug conhecido e documentado (fóruns de suporte da Netlify) do Next.js 14 App Router na runtime da Netlify: `redirect()` lançado de dentro de uma função async **aninhada** (uma função `A` que dá `await` numa função `B`, e é a função `B` quem chama `redirect()`) às vezes não é reconhecido corretamente pela runtime e vira um erro genérico em vez de redirecionar de verdade. `requireRole` fazia exatamente isso — chamava `await requireAuth()`, e era `requireAuth` quem chamava `redirect()` — dois níveis de aninhamento. Toda rota protegida do sistema usa `requireRole`, não `requireAuth` diretamente (só uma exceção, `account-actions.ts`), o que explica por que o problema não seria específico de `/admin`, e sim de qualquer página protegida visitada sem sessão válida.

### DEC-059 — Produção quebrada (continuação): segunda chamada não protegida a `getUser()` na renderização da página

**Data:** 2026-07-15
**Decisão:** `getCurrentProfile()` (`lib/auth/server.ts`, chamada por toda página/layout protegido via `requireRole`) agora também envolve `supabase.auth.getUser()` num `withTimeout()` (8s), tratando timeout como "sem sessão" (retorna `null`, mesmo fallback que erro de rede) em vez de deixar a exceção propagar. O `withTimeout()` foi extraído para `lib/timeout.ts`, compartilhado entre `middleware.ts` e `lib/auth/server.ts` (antes só existia dentro do middleware).
**Motivo:** Depois da correção do `requireRole` aninhado (DEC-058), o erro em produção passou a ser **intermitente** (visto no painel de observabilidade da Netlify: 46,67% de erro em vez de 100%, com uma das tentativas do responsável pelo produto chegando a carregar `/admin` com sucesso). As durações das requisições — inclusive as bem-sucedidas — agrupavam-se por volta de 5 a 5,5 segundos, muito perto do timeout de 5s já aplicado à chamada de `getUser()` do middleware. Isso indicou que o Supabase Auth está genuinely lento a partir do ambiente de produção da Netlify no momento (não só "travando" por cookie obsoleto), e que **cada página protegida paga esse custo duas vezes**: uma vez no middleware (já com timeout) e de novo dentro de `getCurrentProfile()` na própria renderização da página (sem proteção nenhuma até esta correção) — nos casos mais lentos, a soma das duas chamadas estourava o orçamento de execução da function e virava um 500.
**Atualização (mesma data):** o incidente continuou intermitente mesmo depois da correção acima (painel da Netlify mostrou tentativas bem-sucedidas ao lado de outras com o mesmo erro) — evidência de que o Supabase Auth seguia genuinely lento, não só "às vezes travando". Tentei a otimização pendente ali mesmo: `middleware.ts` repassando o `user.id` já resolvido via header de requisição (`x-huios-user-id`), lido em `getCurrentProfile()` via `headers()` de `next/headers` para evitar a segunda chamada de rede. **Essa tentativa foi revertida na sequência** — o responsável pelo produto encontrou nos logs da function da Netlify um `TypeError: (0, i.b) is not a function` em `admin/page.js`, aparecendo assim que essa mudança foi ao ar. `next/headers` já era usado no projeto só via `cookies()` (nunca via `headers()`), e o padrão do erro minificado (import nomeado resolvendo pra `undefined` em runtime) bate com esse export específico não sendo suportado do jeito esperado pela runtime da Netlify para Next.js 14. Revertido para a versão anterior (chamada direta a `getUser()` com timeout, sem o header) — a otimização de eliminar a chamada duplicada fica descartada por ora, não só adiada; se for revisitada, precisa de um mecanismo diferente de `next/headers`'s `headers()`.

### DEC-060 — Causa raiz do 500 em produção: função chamada de um arquivo `"use client"` dentro de um Server Component

**Data:** 2026-07-15
**Decisão:** `getNavItemsForRole()` (introduzida na DEC-051, correção do rodapé de navegação) morava em `components/layout/BottomNav.tsx`, um arquivo `"use client"`. Os três layouts protegidos — `(admin)/layout.tsx`, `(coordination)/layout.tsx`, `(leader)/layout.tsx`, todos Server Components — chamavam essa função diretamente no corpo do componente para montar a lista de itens do rodapé. Extraída para um novo módulo sem `"use client"` (`lib/nav-items.ts`, junto com os tipos `NavItem`/`NavIconName` e a constante `DEFAULT_NAV_ITEMS`); `BottomNav.tsx` agora só exporta o componente React em si, importando os itens/tipos do novo módulo.
**Motivo:** O responsável pelo produto conseguiu, via logs de function da Netlify, o stack trace real por trás do 500 em `/admin`: `TypeError: (0 , i.b) is not a function`, sempre no mesmo ponto do bundle (`admin/page.js:1:9984`) — e notou, corretamente, que esse ponto **não mudou** depois de duas correções consecutivas (DEC-058, DEC-059) que mexiam em `lib/auth/server.ts`, o que indicava que a causa raiz estava em outro lugar da árvore de imports da página, não no fluxo de autenticação. Chamar uma função exportada de um módulo `"use client"` diretamente dentro de um Server Component é um problema conhecido do Next.js App Router: o bundler de produção troca os exports desse módulo por referências de cliente (para o RSC conseguir serializar o que é enviado ao navegador); uma chamada direta a essa referência (em vez de renderizá-la como componente) recebe um objeto que não é a função real, e falha com "X is not a function" — só em build de produção, nunca em `next dev`, e sem gerar erro de tipo ou de lint, porque nada nessa análise estática do TypeScript sabe distinguir "módulo cliente" de "módulo servidor" nesse nível. Como o padrão do erro citado pelo próprio responsável pelo produto ("import nomeado chegando `undefined` em runtime, típico de mismatch client/server component") bate exatamente com essa causa, e como as três chamadas afetadas cobrem justamente as três rotas testadas hoje (`/admin` e as outras duas com o mesmo layout base), esta é considerada a causa raiz mais provável do incidente completo — as correções anteriores (DEC-056 a DEC-059) continuam válidas e necessárias por conta própria, mas nenhuma delas tocava este arquivo.

### DEC-061 — "Mídia" adicionada aos Grupos de Atuação
