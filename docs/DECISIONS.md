...
**Atualização (mesma data):** o incidente continuou intermitente mesmo depois da correção acima (painel da Netlify mostrou tentativas bem-sucedidas ao lado de outras com o mesmo erro) — evidência de que o Supabase Auth seguia genuinely lento, não só "às vezes travando". Em vez de esperar por uma janela calma, a otimização pendente foi feita ali mesmo: `middleware.ts` agora repassa o `user.id` já resolvido via header de requisição (`x-huios-user-id`, string vazia = "sem sessão", ausente = "nunca passou pelo middleware"); `getCurrentProfile()` lê esse header primeiro e só cai para uma chamada própria de `supabase.auth.getUser()` quando o header está ausente (fallback para rotas fora do matcher do middleware, como `/api/admin/export/grs`, que exclui `/api` explicitamente). Isso elimina a segunda chamada de rede ao Supabase Auth em toda página protegida — a causa direta da lentidão dobrada.
